<?php
	...
	class db_class extends db_connect{
		...
		public function save_borrower($firstname,$middlename,$lastname,$contact_no,$address,$email){
			$query=$this->conn->prepare("INSERT INTO `borrower` (`firstname`, `middlename`, `lastname`, `contact_no`, `address`, `email`) VALUES(?, ?, ?, ?, ?, ?)") or die($this->conn->error);
			$query->bind_param("ssssss", $firstname, $middlename, $lastname, $contact_no, $address, $email);
		...
		public function update_borrower($borrower_id,$firstname,$middlename,$lastname,$contact_no,$address,$email){
			$query=$this->conn->prepare("UPDATE `borrower` SET `firstname`=?, `middlename`=?, `lastname`=?, `contact_no`=?, `address`=?, `email`=? WHERE `borrower_id`=?") or die($this->conn->error);
			$query->bind_param("ssssssi", $firstname, $middlename, $lastname, $contact_no, $address, $email,  $borrower_id);
